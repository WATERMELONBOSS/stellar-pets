...
app.post("/goal/create", async (req, res) => {
  try {
    const { walletAddress, petName, petType, goalAmount, depositAmount } =
      req.body;
    ...
    if (!walletAddress || !goalAmount || !depositAmount) {
      return res.status(400).json({
        error:
          "Missing required fields: walletAddress, goalAmount, depositAmount",
      });
    }
...
app.post("/pet/calculate", async (req, res) => {
  try {
    ...
    const newHealth = Math.max(
      0,
      Math.min(100, currentState.health + healthChange)
    );
    const newHappiness = Math.max(
      0,
      Math.min(100, currentState.happiness + happinessChange)
    );
    const newTotalSaved =
      parseFloat(currentState.total_saved) + parseFloat(depositAmount);
    ...
    console.log(
      `Updated pet: health ${newHealth}, happiness ${newHappiness}, streak ${newStreak}`
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to calculate pet state", details: error.message });
  }
});
...
app.post("/withdraw/process", async (req, res) => {
  try {
    ...
    const newTotalSaved = Math.max(
      0,
      parseFloat(currentState.total_saved) - parseFloat(amount)
    );
    ...
    console.log(
      `Withdrawal processed: health ${newHealth}, happiness ${newHappiness}`
    );
...
app.get("/pet/state", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch pet state", details: error.message });
  }
});
